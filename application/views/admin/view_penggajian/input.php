...
<section class="content">
	...
	<div class="box">
		<form class="form-horizontal" action="<?php echo base_url("detail/tambah_proses/$id_proyek"); ?>" method="post">
			<div class="box-body">
					...
					<div class="col-md-6">
						<select class="form-control" id="jobdesk" name="jobdesk" onchange="persen()" required>
							...
							<?php foreach ($data_jobdesk as $k) { ?>
								<option value="<?= $k->id_jobdesk ?>"><?= $k->jobdesk . " | " . $k->persen_upah . "%" ?></option>
							<?php } ?>
						</select>
					</div>
				</div>
